const { joox } = require('../function/scrape')
let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (!text) return conn.reply(m.chat, `• *Example :* ${usedPrefix + command} akad`, m)
    if (isUrl(text)) return conn.reply(m.chat, `• *Example :* ${usedPrefix + command} akad`, m)
...
handler.help = ['joox'].map(v => v + ' *<text>*')
