...
let handler = async (m, { conn, isOwner, usedPrefix, command, args }) => {
    ...
    if (command === "vace") {
        let res = [
            "adios", "batmanslap", "carreverse", "changemymind", "distractedbf",
            "dockofshame", "drip", "ejected", "emergencymeeting", "firsttime",
            "grave", "heaven", "iamspeed", "icanmilkyou", "npc", "peeposign",
            "rankcard", "stonks", "tableflip", "water", "wide", "wolverine", "womanyellingatcat"
        ];
        ...
        let listSections = res.map((v, index) => ({
            ...
            rows: [{
                title: v.toUpperCase(),
                description: "",
                id: usedPrefix + "vaceget " + v + "|" + text
            }]
        }));

        return conn.sendListMsg(m.chat, "📺 Vace Maker 🔎", `⚡ Silakan pilih Model di tombol di bawah...\n*Teks yang anda kirim:* ${text}\n\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`, "☂️ M O D E L ☂️", listSections, m);
    }

    if (command === "vaceget") {
        let res = await Vace(one, two, three, four, five, six);
        await conn.sendFile(m.chat, res, 'vace.jpg', 'Here is your Vace Maker result!', m, false);
    }
};
