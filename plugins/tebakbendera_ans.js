...
handler.before = async function (m) {
    ...
    if (m.quoted.id == this.tebakbendera[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.name.toLowerCase().trim()) {
            ...
            await this.sendImageAsSticker(m.chat, betul, m, { packname: "sticker by", author: "©DitzOfc" })
            await m.reply(dpoint)
            clearTimeout(this.tebakbendera[id][3])
            delete this.tebakbendera[id]
        } else 
        await this.sendImageAsSticker(m.chat, salah, m, { packname: "sticker by", author: "©DitzOfc" })
        await m.reply(mpoint)
    }
    return !0
}
handler.exp = 0
