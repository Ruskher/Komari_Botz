...
return this.sendButtonMsg(m.chat,  '```🚩 Akses Bot Ke Private Chat Di Tolak, Upgrade Premium Hanya Rp 5.000 Agar Bisa Bebas Akses Bot Dengan Hubungi Owner : ```' + `@${nomorown.split("@")[0]}` + '```\n\n• Join Ke Channel Informasi Bot Untuk Update Kedepannya Tentang Bot : https://whatsapp.com/channel/0029VaYyCtsHltYBaNA0DQ36```', '```©DitzOfc 2024```', [{ text: '', id:'' }], m)
                    ...
                    try {
                        ...
                        if (e) {
                            ...
                            for (let key of Object.values(global.APIKeys))
                                text = text.replace(new RegExp(key, 'g'), 'Tenka')
...
global.dfail = async (type, m, conn) => {
    ...
    if (type === 'admin') {
        let stickerBuffer = fs.readFileSync('./admin.webp');
    ...
    let msgg = {
       unreg: Func.Styles('Hello, before using the bot you have to register first.\n\nhow to register?\n\nCaptcha Verify\n*example:* .register adit.18\n')
    }[type]
