...
class Wattpad {
	constructor() {
        ...
        this.resultType = ['trending', 'popular', 'featured', 'new']
	}
	search(query) {
		return new Promise(async (resolve, reject) => {
			await axios.request({
				...
				// variabel results dari contohnya bakal dibuat variatif
				data: anu.data.stories, 
                resultType: this.resultType[Math.floor(Math.random() * this.resultType.length)] 
			})).catch(reject);
		});
	}
	story(url) {
